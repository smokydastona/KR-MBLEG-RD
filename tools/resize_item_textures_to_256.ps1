...
# Resizes item textures to 256x256.
# Targets:
#  1) assets/kruemblegard/textures/item
# Rules:
...
# Report behavior:
# - Default: overwrites "item_textures_*_latest.txt" and deletes older timestamped reports.
...
if ($KeepHistory) {
  $changedReportPath = Join-Path $ReportDir ("item_textures_changed_{0}.txt" -f $timestamp)
  $nonSquareReportPath = Join-Path $ReportDir ("item_textures_non_square_{0}.txt" -f $timestamp)
} else {
  $changedReportPath = Join-Path $ReportDir 'item_textures_changed_latest.txt'
  $nonSquareReportPath = Join-Path $ReportDir 'item_textures_non_square_latest.txt'
}
...
if (-not $NoReports) {
  ...
  if (-not $KeepHistory) {
    ...
    Get-ChildItem -Path $ReportDir -File -Filter 'item_textures_changed_*.txt' -ErrorAction SilentlyContinue |
      Where-Object { $_.Name -ne 'item_textures_changed_latest.txt' } |
      Remove-Item -Force -ErrorAction SilentlyContinue
    Get-ChildItem -Path $ReportDir -File -Filter 'item_textures_non_square_*.txt' -ErrorAction SilentlyContinue |
      Where-Object { $_.Name -ne 'item_textures_non_square_latest.txt' } |
      Remove-Item -Force -ErrorAction SilentlyContinue
  }
