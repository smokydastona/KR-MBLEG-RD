...
def generate_pressure_sequencer() -> GeneratedBlockAssets:
    # Concept-level spec in docs/PRESSURE_LOGIC.md: redstone pulses -> pressure pulses.
    # No UV blueprint provided yet; keep this minimal + palette-locked.
    size = 32
    ceramic_light = PALETTE["ceramic"]["light"]
    ceramic_mid = PALETTE["ceramic"]["mid"]
    ...
    def make_front() -> list[list[str]]:
        ...
        for y in range(size):
            for x in range(size):
                if not ring[y][x]:
                    continue
                g[y][x] = ceramic_dark if not _is_outline(ring, x, y) else ceramic_deep
        ...
        for dx, dy in ((0, -9), (9, 0), (0, 9), (-9, 0)):
            ...
            y0 = 16 + dy
            for i in range(3):
                xx = x0 + (0 if dy else (1 if dx > 0 else -1)) * i
                yy = y0 + (0 if dx else (1 if dy > 0 else -1)) * i
                if 0 <= xx < size and 0 <= yy < size:
                    g[yy][xx] = ceramic_line

        # Crystal indicator bar.
        for y in range(10, 22):
            g[y][24] = crystal_shadow
            g[y][25] = crystal_mid
        g[15][25] = glow
    ...
    return GeneratedBlockAssets(
        block_id="pressure_sequencer",
        face_grids=face_grids,
        item_grid=item_grid,
        horizontal_facing=True,
        powered=True,
        enum_variants={"step": ["0", "1", "2", "3"]},
    )
