...
- Behavior notes: Scaralon eggs use vanilla turtle-style clutch stacking on the same block up to 4 eggs; larva cannot fly; if no adult Scaralon is nearby they now seek reachable trunk/log faces with nearby leaves, climb onto the bark, and sap-suck there to halve maturation time until an adult returns or they are interrupted.
