...
- **Cephalari** (`cephalari`)
  ...
  - Visual (adult-form appearance): the base render uses a Cephalari golem texture on non-`cephalari` bones (body), and overlay passes re-texture only the embedded `cephalari` subtree using the Cephalari body texture plus profession + badge overlays on their dedicated bones.
    ...
    - Visual (adult-form shells): all 4 adult-form shells use the shared golem-atlas path, and the renderer now forces a deterministic outer-shell versus embedded-Cephalari draw order so Spiral Strider and DriftSkimmer do not collapse visually to the inner Cephalari layer.
