...
- **Cephalari** (`cephalari`)
  ...
  - Visual (adult-form appearance): the base render uses a Cephalari golem texture on non-`cephalari` bones (body), and overlay passes re-texture only the embedded `cephalari` subtree using the Cephalari body texture plus profession + badge overlays on their dedicated bones.
    ...
    - Visual (adult-form shells): all 4 adult-form outer-form bones use the shared golem-atlas path, and the stand-alone adult-form renderer now applies the embedded `cephalari` overlay passes before the outer-form base draw while still skipping base cubes on the embedded `cephalari` subtree.
