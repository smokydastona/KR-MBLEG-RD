...
- **Ancient Way Ruins (Overworld structure)**
  ...
  - Aging pass: placement applies a structure-specific ruin processor pass that strips red-wool marker blocks to air, chips blocks away, mosses/cracks masonry, fades about twenty percent of `purple_terracotta` into `pink_terracotta`, mineralizes about twenty percent of quartz accents into `diorite`, and converts `sculk_vein` markers into a lighter ancient-city-style mix where about half the marker sculk weathers back into stone-brick/cobble ruin masonry, about ten percent of the remaining sculk becomes blackstone variants, and the rest mostly becomes air or `sculk_sensor` blocks with comparatively sparse Warden-capable `sculk_shrieker` blocks.
