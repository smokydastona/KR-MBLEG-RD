...
## Localization
- Krümblegård ships language JSON files for every Minecraft Java 1.20.1 supported locale so the mod remains readable under any built-in language selection instead of exposing raw translation keys.
...
- Use `python tools/translate_lang_locales.py` only to draft placeholder text where English fallback entries still need later review. If compound mod terms are coming through untranslated, try `--hint-compounds` to split certain protected compounds (e.g., `Traprock` -> `Trap rock`) as a translation hint.
...
- **Ancient Way Ruins (Overworld structure)**
  ...
  - Aging pass: placement applies a structure-specific ruin processor pass that strips red-wool marker blocks to air, chips blocks away, mosses/cracks masonry, fades about twenty percent of `purple_terracotta` into `pink_terracotta`, mineralizes about twenty percent of quartz block accents into `diorite`, weathers a small share of `quartz_pillar` into `polished_diorite`, and uses explicit palette-state matches so quartz stairs and slabs weather into matching `polished_diorite_stairs` and `polished_diorite_slab` states without losing facing, half, shape, or waterlogging, while converting `sculk_vein` markers into a lighter ancient-city-style mix where about half the marker sculk weathers back into stone-brick/cobble ruin masonry, about ten percent of the remaining sculk becomes blackstone variants, and the rest now skews much harder toward air with a thinner `sculk_sensor` residue and genuinely rare Warden-capable `sculk_shrieker` blocks.
