...
- `wayfall_portal`: a simple testing portal block you can place in creative; entities (players/mobs/items) that pass through it are sent to the Wayfall spawn.
  ...
  - Template landing marker: put a single `minecraft:barrier` block in the template at the intended **feet** position; the game teleports the player above it and removes the marker.
...
- Safety: entering Wayfall via the portal always lands you on a dedicated spawn island (a small tapered "asteroid") at Wayfall's shared spawn X/Z (with cleared headroom).
