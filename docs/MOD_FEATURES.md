...
- Blocks:
  ...
  - `pressure_loom`: Flowwright workstation that runs in stable air when powered and supplied with conduit pressure + rotation; weaves Paleweft into `coral_fiber`, fuses Attuned Stone + Volatile Resin into `bio_ceramic`, and forges `scarsteel_ingot` from 4 `scarstone` plus 1 `attuned_ingot`. The three recipe costs are now tuned around the same starter-session budget: basic weaving is cheapest, Bio-Ceramic is mid-cost, and a single coal-backed Thermo-Condenser loop should land around 2 Scarsteel crafts if the line stays stable (outputs as item entities above the block).
