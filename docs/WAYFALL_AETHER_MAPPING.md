...
- ⚠️ Not an exact copy: Wayfall adds its own `island_shape` (2D elevation + 3D noise) and a `mid_bulge` term that is tuned to keep islands playable above the Y=130 void cutoff while preserving air gaps.

#### Splines (Y-Slides)
- ✅ `mid_bulge.json`: ramps up after Y=130, then fades back down at higher Y so the dimension doesn’t fill solidly to build limit.
