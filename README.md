...
- **Ancient Way Ruins**: a new red-wool-anchored overworld ruin structure sourced directly from `wayfall_temple.schem`, globally limited to three concentric-ring placements per world, hard-blocked from generating within 2048 blocks of world origin, kept away from vanilla strongholds via a structure-set exclusion zone, weathered during placement, mapped by master cartographers, seeded with ancient-city-style sculk sensor/shrieker conversions, and restricted to Cephalari zombie-family monster spawns inside its bounding box while summoned Wardens remain valid.
