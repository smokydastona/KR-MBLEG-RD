...
- **Telekinesis enchantment**: a new very-rare mainhand enchantment for breakable tools and weapons that routes block drops and both direct or projectile kill drops into the attacking player's inventory when space allows, spills only leftovers at the player, and plays the vanilla pickup cue when items are absorbed.
