...
- **Telekinesis enchantment**: a new very-rare enchanting-table/anvil enchantment for tools and weapons only, routing block drops and both direct or projectile kill drops from the enchanted item into the attacking player's inventory when space allows, spilling only leftovers at the player, playing the vanilla pickup cue on successful absorption, appearing in End City and Ancient City loot both as enchanted books and pre-enchanted gear, selling from master librarians as a book, and occasionally rolling onto master toolsmith/weaponsmith stock.
