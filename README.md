...
- Strict mod-term gate (default): `./tools/sync_lang_files.ps1 -Verify` fails if specific mod name-terms (like `Waystone`) remain effectively English (exact match, or only differing by the zero-width-space coverage marker).
