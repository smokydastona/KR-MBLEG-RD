...
- **Cephalari adult forms**: Spiral Strider, DriftSkimmer, Treadwinder, and Echo Harness all share the embedded Cephalari body/profession texture mapping path, and their stand-alone renderer now follows GeckoLib's default shell-bone recursion while only suppressing base cubes on the embedded `cephalari` subtree so the shell and rider layers can render together.
