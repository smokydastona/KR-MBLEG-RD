...
- **Cephalari adult forms**: Spiral Strider, DriftSkimmer, Treadwinder, and Echo Harness all share the embedded Cephalari body/profession texture mapping path, and their stand-alone renderer now renders the embedded `cephalari` root before the other root-level outer-form bones while preserving GeckoLib's recursive bone handling, so the outer-form bones and embedded `cephalari` bones can render together.
