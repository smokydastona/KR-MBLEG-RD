...
- **Cephalari adult forms**: Spiral Strider, DriftSkimmer, Treadwinder, and Echo Harness all share the embedded Cephalari body/profession texture mapping path, and their stand-alone renderer now applies the embedded Cephalari overlay passes before the outer-form base draw so the outer-form bones and embedded `cephalari` bones can render together.
