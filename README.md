...
- **Wayfall bats**: vanilla bats now also receive a Wayfall biome-spawn injection tuned to the same lighter ambient pressure band as Pebble Wrens, so they share the biome coverage there without swamping the ambient cap; Overworld bat spawning stays on vanilla rules instead of being double-added by the mod.
