...
- Fix(render/adult_forms): realign the stand-alone adult-form renderer with GeckoLib's default recursive shell rendering and matrix tracking while still skipping base cubes on the embedded `cephalari` subtree, so Spiral Strider and DriftSkimmer can render shell and rider layers together without changing the authored UVs or current textures.
