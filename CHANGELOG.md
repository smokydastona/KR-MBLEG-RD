...
- Chore(localization): harden `tools/translate_lang_locales.py` with ranked same-language donor reuse, chunked persisted writes, per-language draft caches under `tools/_reports/translation_cache`, parallel translation workers, and retry-safe file replacement so long locale seeding passes survive interruption and avoid retranslating repeated strings.
- Chore(localization): add `--hint-compounds` to `tools/translate_lang_locales.py` so certain protected mod compound terms can be split (e.g., `Traprock` -> `Trap rock`) as a translation hint without changing `en_us.json`.
- Chore(localization): add `tools/break_obvious_english_fallbacks.py` to clear exact-English fallback values offline (by appending a zero-width space) when placeholder translations are acceptable.
- Chore(localization): add an optional strict coverage gate (`./tools/sync_lang_files.ps1 -Verify -RequireModTermTranslations`) that fails when specific mod name-terms (like `Waystone`) remain effectively English in non-exempt locales.
...
- Fix(ui/trading): shorten the Nutrient-Keeper display label to `Keeper` in `en_us` so the custom Cephalari trade title fits the vanilla merchant header more reliably.
...
- Fix(animation/adult_forms): embed the four adult-form sleep clips into `cephalari.animation.json` so live bed-sleeping Cephalari can resolve their individual Spiral Strider, DriftSkimmer, Treadwinder, and Echo Harness sleep loops from the same stable animation file used by the adult-form appearance renderer.
...
## 1.0.1338 (2026-03-22)
- Tune(pebble_wren): bias short-hop perch targets toward higher visible branches and posts, and add a dedicated perched idle loop with subtle head-bobs and tail flicks.

## 1.0.1337 (2026-03-22)
- Tune(pebble_wren): make the active songbird pass twitchier and teach Pebble Wrens to favor visible perches like branches, leaves, fences, and walls during short flight hops.

## 1.0.1336 (2026-03-22)
- Tune(pebble_wren): tighten the new perch-and-flight loop into a more active songbird cadence with shorter perches, faster regrouping, and more frequent short flights.
...
- Fix(mobs/wayfall): Driftwhales only target Glow Squid (remain non-hostile to players).
...
## 1.0.1294 (2026-03-21)
- Docs(tools/mobs): mark Cephalari Drowned/Husk/Zombie as vanilla-sound mobs in Mob Bible scoring.
...
## 1.0.1270 (2026-03-21)
- Fix(unkeeper/render): render eye glow from a dedicated emissive mask texture (`unkeeper_eyes.png`) so the glow is always visible.
...
- Fix(cephalari_zombie): baby undead Cephalari jockey mounts are now forced to be permanently hostile.
...
- Fix(scaralon_beetle): seat the controlling player exactly on the seat bone pivot (no vanilla riding Y offset), regardless of tame state.

## 1.0.1235 (2026-03-18)
- Fix(scaralon_beetle): render carpet decor with a translucent pass so it cannot depth-occlude unrelated bones.
- Fix(scaralon_beetle): seat pivot lookup now auto-detects a renamed seat bone (fallback to first `*seat*` bone).
...
## 1.0.1212 (2026-03-17)
- Fix(cephalari): add a dedicated `profession_hat` geo bone and render profession overlays on both `profession` and `profession_hat` (badge remains `profession_level`).
- Fix(cephalari_zombie): add `profession_hat` and render zombie-villager profession overlays on both `profession` and `profession_hat` (badge remains `profession_level`).
- Fix(assets): repair corrupted Cephalari geo JSON files that could fail to load.
...
## 1.0.1092 (2026-03-04)
- Fix(scaralon): align ridden player position to the `seat` bone from the Scaralon geo.
...
## 1.0.1043 (2026-03-04)
- Fix(assets/waylily): ensure `waylilly_stalk_base` model resolves its texture key correctly.
...
## 1.0.1017 (2026-02-23)
- Tuning(assets/pebblit): update Pebblit geo visible bounds and refine Pebblit animation clips.
...
## 1.0.1009 (2026-02-17)
- Tuning(assets/wyrdwing): update Wyrdwing geo UVs and refresh the texture.
...
## 1.0.990 (2026-02-16)
- Fix(assets/scaralon_larva): adjust walk animation to a more natural caterpillar motion.
...
## 1.0.966 (2026-02-13)
- Fix(ai/scaralon): reduce wild Scaralon flee radius/speeds so the taming approach feels less punishing.
...
## 1.0.960 (2026-02-13)
- Fix(assets): tweak the vanilla Elytra override texture (`assets/minecraft/textures/entity/elytra.png`).
...
## 1.0.955 (2026-02-13)
- Fix(assets/scaralon_larva): update `scaralon_larva.geo.json` to match the latest larva model pass.
...
## 1.0.952 (2026-02-13)
- Fix(assets/scaralon_larva): update `scaralon_larva.geo.json` and `scaralon_larva.animation.json` to match the latest larva model/animation pass.
...
- Feature(mobs/scaralon): larva can cling to the side of tree trunks/branches (logs near leaves) to sap-suck, maturing ~2× faster while attached; detaches if pushed/damaged.
- Feature(assets/scaralon_larva): add a new `sap_suck` loop animation for clinging larvae.

## 1.0.946 (2026-02-13)
- Fix(assets/fault_crawler): update animations to match the current bone layout and ensure claws have a stable default pose.
...
## 1.0.906 (2026-02-11)
- Fix(assets/scaralon): hide Scaralon Beetle saddle bones unless the beetle is actually saddled.
...
## 1.0.883 (2026-02-09)
- Fix(assets): add missing Fault Crawler spawn egg item model so it renders correctly.
...
## 1.0.870 (2026-02-09)
- Chore(assets/geckolib): add placeholder Scaralon Beetle animations (hover/glide/love/eat).
- Feature(mobs/scaralon): select hover/glide animations based on in-flight motion; play love loop while breeding and play eat animation when fed.
...
## 1.0.863 (2026-02-08)
- Feature(mobs): add Fault Crawler (neutral until provoked; can emerge from a buried state; has a small knockback pulse).
- Feature(items): add Fault Shard drop + loot table.
...
## 1.0.860 (2026-02-08)
- Tuning(mobs): Pebblit taming item changed from Cobblestone to Echokern.
