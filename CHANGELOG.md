...
- Fix(render/adult_forms): render the embedded `cephalari` root before the outer shell root siblings in the stand-alone adult-form renderer so Spiral Strider and DriftSkimmer stop drawing like rider-only bodies while leaving the authored UVs and current textures intact.
